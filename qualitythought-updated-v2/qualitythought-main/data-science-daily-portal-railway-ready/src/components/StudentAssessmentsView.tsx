...
import { CodeCompiler } from "./CodeCompiler.js";
        ...
        <div className="p-6 space-y-8">
          <div className="bg-emerald-50/50 rounded-xl p-4 border border-emerald-150 flex items-center justify-between">
            <div className="space-y-0.5">
              <span className="text-[10px] uppercase font-mono tracking-wider text-emerald-900 font-bold block">Final Status</span>
                      ...
                      return (
                        ...
                        >
                          <div className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 text-[10px] font-bold ${
                            isCorrect ? "bg-emerald-600 border-emerald-600 text-white" : "border-slate-300 bg-slate-50 text-slate-400"
                          }`}>
                            {isCorrect ? "✓" : ""}
                          </div>
                          <span>{opt}</span>
              ...
              {currentPreset.coding.map((q, idx) => (
                <div key={idx} className="bg-slate-50/50 p-5 rounded-xl border border-slate-200 space-y-3">
                  ...
                  <div className="space-y-1">
                    <span className="text-[10px] font-bold text-emerald-900 block uppercase font-mono">
                      Expected Validation Keywords:
                    </span>
                    <div className="flex flex-wrap gap-1.5 pt-0.5">
                      {q.expectedKeywords.map((kw, kwIdx) => (
                        <span key={kwIdx} className="font-mono text-[9px] bg-slate-200 text-slate-700 px-2 py-0.5 rounded font-bold border border-slate-300/40">
                          "{kw}"
                        </span>
                      ))}
                    </div>
                ...
                {activeSched.coding.map((q: any, idx: number) => (
                  <div key={idx} className="bg-slate-50/60 p-5 rounded-xl border border-slate-200 space-y-3">
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span className="font-mono text-[9px] bg-slate-200 bg-slate-200 text-slate-700 px-2.5 py-0.5 rounded-full font-black border border-slate-300/40">
                        Syllabus Checkpoint
                      </span>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      <span className="text-[10px] font-bold text-indigo-800 text-indigo-900 block uppercase font-mono">
                      ...
                      />
                      <CodeCompiler code={schedCodingAnswers[idx] || ""} compact />
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div key={idx} className="bg-slate-50/50 p-5 rounded-xl border border-slate-200 space-y-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea
                      ...
                      placeholder="def solution_fn():..."
                    />
                    <CodeCompiler code={codingAnswers[idx] || ""} compact />
                  </div>
                </div>
              ))}
            </div>
      ...
      {subTab === "scheduled" ? (
        <div className="space-y-6 animate-fade-in">
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="bg-indigo-50 p-3 rounded-xl border border-indigo-200 text-xs shrink-0 font-medium font-mono text-indigo-800">
          ...
          <div className="flex flex-wrap border border-slate-200 gap-1 bg-slate-50/50 p-1 rounded-xl w-full sm:w-fit text-left">
            <button
            ...
            if (filteredScheduledTestsBySubtype.length === 0) {
              return (
                <div className="bg-white rounded-xl border border-slate-200 p-12 text-center text-slate-400 text-xs space-y-1">
                  ...
                  <p className="text-[10px] text-slate-400">Wait for your course administrator Vinay to launch new scheduled questions.</p>
                </div>
              );
            }
                      ...
                      <div className="pt-3.5 border-t border-slate-100 flex items-center justify-between">
                        {sub ? (
                          ...
                          <button
                            ...
                            className="text-xs font-mono font-bold py-2 px-5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white shadow-xs transition-all flex items-center gap-1 cursor-pointer"
                          >
          ...
          return (
            <div
              ...
              className={`bg-white rounded-xl border p-5 shadow-xs flex flex-col justify-between gap-5 transition-all ${
                isUnlocked ? "border-slate-300 hover:shadow-md animate-fadeIn" : "border-slate-200 bg-slate-50/55"
              }`}
            >
              <div className="space-y-3">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex justify-between items-center w-full">
                    {scoreRecord ? (
                      ...
                      <button
                        ...
                        disabled={!hasPreset}
                        className="text-xs font-bold py-2 px-5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white border border-transparent shadow-sm transition-all flex items-center gap-1.5 cursor-pointer disabled:bg-slate-300 disabled:cursor-not-allowed"
                      >
