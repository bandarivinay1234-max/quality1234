...
export default function TeacherDashboard({ onLogout }: TeacherDashboardProps) {
  ...
  const handleSetClassZoomLink = async () => {
    const link = classZoomLink.trim();
    if (!link) return;
    ...
    try {
      ...
      console.error("Failed to set class zoom link:", err);
    }
  };
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col xl:flex-row gap-4 items-center justify-between print:hidden">
                  ...
                  <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto justify-center xl:justify-end">
                    ...
                    {/* Set class-wide Zoom link */}
                    <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-2 py-1">
                      <input
                        type="text"
                        value={classZoomLink}
                        onChange={(e) => setClassZoomLink(e.target.value)}
                        placeholder="Paste today's Zoom link"
                        className="text-xs font-mono px-1.5 py-1 outline-none w-44"
                      />
                      <button
                        type="button"
                        onClick={handleSetClassZoomLink}
                        disabled={!classZoomLink.trim()}
                        className="bg-purple-600 hover:bg-purple-700 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-md px-2.5 py-1 text-[11px] font-bold transition cursor-pointer whitespace-nowrap"
                      >
                        🔗 Set Class Zoom Link
                      </button>
                    </div>
