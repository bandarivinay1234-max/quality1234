...
import { getTestCasesForQuestion } from "../utils/testCases.js";
...
export default function StudentPortal({ student, onLogout }: StudentPortalProps) {
  ...
  const [activeTab, setActiveTab] = useState<"curriculum" | "assessments" | "interview" | "careers" | "resume" | "compiler" | "videos">("curriculum");
                            ...
                            {quizData.coding.map((codingQ, cIdx) => {
                              ...
                              return (
                                <div key={cIdx} className="space-y-3 pt-4">
                                  ...
                                  {/* ✅ Run Your Code For Real — Online Python Compiler (replaces simulated checking) */}
                                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3 font-sans text-left">
                                    <span className="text-[10px] font-extrabold text-slate-500 tracking-wider uppercase block font-mono">
                                      ▶ Run Your Submitted Code (Real Python Execution)
                                    </span>
                                    <CodeCompiler code={userCode} compact />
                                  </div>

                                  <div className="bg-emerald-50/40 border border-emerald-200 rounded-xl p-4 space-y-3 font-sans text-left">
                                    <span className="text-[10px] font-extrabold text-emerald-700 tracking-wider uppercase block font-mono">
                                      ▶ Run The Ideal Model Solution (Real Python Execution)
                                    </span>
                                    <CodeCompiler code={codingQ.solutionDescription} compact />
                                  </div>
